...
class DDIMSampler(object):
    def __init__(self, model, merge_mode=0,schedule="linear", **kwargs):
        super().__init__()
        self.model = model
        self.ddpm_num_timesteps = model.num_timesteps
        self.schedule = schedule
        self.noise = torch.load('xt.pt')
    ...
    @torch.no_grad()
    def ddim_inversion(self, latent,batch_size):
        self.noise = self.noise[:batch_size]
        all_latent = [latent]
    ...
    def sample(self,
        ...
        if self.merge_mode not in [0,1]:
            x_T = self.ddim_inversion(ref_img,batch_size)[-1]
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        start_t = mykwargs['start_t']
        back_t = mykwargs['back_t']
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            elif self.merge_mode == 1:  # rollback disturbance for free generation
                self.pre_x.append(x)
                t_in = torch.cat([t] * 2)
                c_in = torch.cat([unconditional_conditioning, c])
                if t[0]>start_t:
                    ...
                    x_in = torch.cat([self.pre_x[-back_t]] + [x])
                e_t_uncond, e_t = self.model.apply_model(x_in, t_in, c_in).chunk(2)
                e_t = e_t_uncond + unconditional_guidance_scale * (e_t - e_t_uncond)
            elif self.merge_mode == 2:#  rollback disturbance for Image cartoonization
                self.pre_x.append(x)
                t_in = torch.cat([t] * 2)
                c_in = torch.cat([unconditional_conditioning, c])
                if t[0]>start_t:
                    ...
                    # back_t_b = 0 if back_t > (1 - t[0]) else back_t
                    x_in = torch.cat([self.pre_x[-back_t]] + [x])
                    e_t_uncond, e_t = self.model.apply_model(x_in, t_in, c_in).chunk(2)
                    e_t = e_t_uncond + unconditional_guidance_scale * (e_t - e_t_uncond)
            elif self.merge_mode == 3:  # Image disturbance for Image cartoonization
                t_in = torch.cat([t] * 2)
                c_in = torch.cat([unconditional_conditioning, c])
                if t[0] > start_t:
                    e_t = self.noise
                else:
                    x_in = torch.cat([self.ref_img] + [x])
                    e_t_uncond, e_t = self.model.apply_model(x_in, t_in, c_in).chunk(2)
                    e_t = e_t_uncond + unconditional_guidance_scale * (e_t - e_t_uncond)
